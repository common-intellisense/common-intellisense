import data from './vantWeapp.data.json'
...
function run() {
  ...
  const r = {}
  Array.from(document.querySelectorAll('.van-doc-card>h3')).forEach(item => {
    if (item.id.endsWith('props')) {
      ...
      Array.from(item.parentElement.querySelectorAll('tbody tr')).forEach((item) => {
        const name = item.children[0].textContent.split(' ')[0]
        const version = item.children[0].textContent.split(' ').slice(1).join((' '))
        const required = item.children[0].textContent.includes('*')
    ...
    } else if (item.id.endsWith('event')) {
      let _name = item.textContent.split(' ')[0]
  ...
  for (const key in r) {
    ...
    const name = `Van${baseName}`
    const result = { name, props: r[key].props, link, link_zh: link.replace('en-US', 'zh-CN'), typeDetail: {}, events: r[key].events, methods: r[key].methods, slots: r[key].slots, suggestions: [], filename: baseName }
    console.log(result)
    results.push(result)
  }
...
function arrayGenerateFile(array) {
  const baseSrc = 'src/ui/vantWeapp/vantWeapp1'
